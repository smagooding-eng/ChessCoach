...
- **ELO Progress Tracker**: Visual ELO progress indicator in the sidebar and mobile header showing rating change since first imported game. Displays a mini sparkline chart (sampled to 30 points) and delta badge (green for gains, red for losses). API: `GET /api/games/elo-progress?username=`. Hook: `useEloProgress`. Components: `EloTrackerBadge` (sparkline + delta), `EloTrackerInline` (delta only).
