...
- **Engine**: Lichess Cloud Eval API (Stockfish) for move classification + centipawn loss. Rate-limited sequential requests (350ms min delay) with retry/backoff on 429s and in-memory FEN cache. postProcessReview correction pass. Accuracy uses chess.com formula: `103.1668 * exp(-0.04354 * avgWinPctLoss) - 3.1668`. Backend converts centipawn evals to win% via `winPct(cp) = 50 + 50*(2/(1+exp(-0.00368208*cp))-1)`, sends per-move win% loss (not raw cp) in `cpLoss` field. Game Rating uses piecewise-linear anchor table matching chess.com's scale. Unverified moves (no engine) get 3% win-loss floor penalty.
