...
- **Engine**: Local Stockfish 17 binary (installed via nix) for move evaluation. Spawned as persistent child process via UCI protocol, evaluates ALL positions at depth 16 with MultiPV 2. `evaluateAllPositions()` in `engineAnalysis.ts` runs sequential evals. `postProcessReview` iterates over ALL original moves (not just GPT output), computes cpLoss/classification from engine data. Accuracy uses chess.com formula: `103.1668 * exp(-0.04354 * avgWinPctLoss) - 3.1668`. Backend converts centipawn evals to win% via `winPct(cp) = 50 + 50*(2/(1+exp(-0.00368208*cp))-1)`, sends per-move win% loss (not raw cp) in `cpLoss` field. Game Rating uses piecewise-linear anchor table matching chess.com's scale. Every move gets `engineAvailable: true` since Stockfish evaluates every position locally.
