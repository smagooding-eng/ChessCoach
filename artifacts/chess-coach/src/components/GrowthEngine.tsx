...
function CredentialSettings() {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-[2rem] border border-border/40 bg-card overflow-hidden">
      ...
      <div className="p-4 space-y-4">
        {loading ? <div className="flex justify-center py-4"><Loader2 className="w-5 h-5 animate-spin text-muted-foreground" /></div> : (
          <>
            <div className="rounded-3xl border border-border/30 p-3 space-y-2">
              ...
              <input value={discordWebhook} onChange={e => setDiscordWebhook(e.target.value)} placeholder="Webhook URL (https://discord.com/api/webhooks/...)" className="w-full bg-background border border-border/30 rounded-[2rem] px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-blue-500/30" />
              <div className="flex gap-2">
                <button onClick={() => saveCredential('discord', { webhookUrl: discordWebhook })} disabled={!discordWebhook || saving === 'discord'} className="flex items-center gap-1 px-3 py-1.5 rounded-[2rem] bg-blue-500/15 border border-blue-500/25 text-blue-400 text-[10px] font-bold hover:bg-blue-500/25 disabled:opacity-40">
                  {saving === 'discord' ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />} Save
                </button>
                {isConfigured('discord') && <>
                  <button onClick={() => testCredential('discord')} disabled={testing === 'discord'} className="flex items-center gap-1 px-3 py-1.5 rounded-[2rem] bg-background border border-border/30 text-foreground text-[10px] font-bold hover:bg-muted/50 disabled:opacity-40">
                    {testing === 'discord' ? <Loader2 className="w-3 h-3 animate-spin" /> : <Send className="w-3 h-3" />} Test
                  </button>
                  <button onClick={() => deleteCred('discord')} className="flex items-center gap-1 px-3 py-1.5 rounded-[2rem] text-red-400 text-[10px] font-bold hover:bg-red-500/10"><Trash2 className="w-3 h-3" /></button>
                </>}
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-border/30 p-3 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <input value={twitterKeys.apiKey} onChange={e => setTwitterKeys(p => ({...p, apiKey: e.target.value}))} placeholder="API Key" className="bg-background border border-border/30 rounded-[2rem] px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-blue-500/30" />
                <input value={twitterKeys.apiSecret} onChange={e => setTwitterKeys(p => ({...p, apiSecret: e.target.value}))} placeholder="API Secret" type="password" className="bg-background border border-border/30 rounded-[2rem] px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-blue-500/30" />
                <input value={twitterKeys.accessToken} onChange={e => setTwitterKeys(p => ({...p, accessToken: e.target.value}))} placeholder="Access Token" className="bg-background border border-border/30 rounded-[2rem] px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-blue-500/30" />
              ...
              <div className="flex gap-2">
                <button onClick={() => saveCredential('twitter', twitterKeys)} disabled={!twitterKeys.apiKey || saving === 'twitter'} className="flex items-center gap-1 px-3 py-1.5 rounded-[2rem] bg-blue-500/15 border border-blue-500/25 text-blue-400 text-[10px] font-bold hover:bg-blue-500/25 disabled:opacity-40">
                  {saving === 'twitter' ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />} Save
                </button>
                {isConfigured('twitter') && <>
                  <button onClick={() => testCredential('twitter')} disabled={testing === 'twitter'} className="flex items-center gap-1 px-3 py-1.5 rounded-[2rem] bg-background border border-border/30 text-foreground text-[10px] font-bold hover:bg-muted/50 disabled:opacity-40">
                    {testing === 'twitter' ? <Loader2 className="w-3 h-3 animate-spin" /> : <Send className="w-3 h-3" />} Test
                  </button>
                  <button onClick={() => deleteCred('twitter')} className="flex items-center gap-1 px-3 py-1.5 rounded-[2rem] text-red-400 text-[10px] font-bold hover:bg-red-500/10"><Trash2 className="w-3 h-3" /></button>
                </>}
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-border/30 p-3 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <input value={redditKeys.clientId} onChange={e => setRedditKeys(p => ({...p, clientId: e.target.value}))} placeholder="Client ID" className="bg-background border border-border/30 rounded-[2rem] px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-blue-500/30" />
                ...
                <input value={redditKeys.username} onChange={e => setRedditKeys(p => ({...p, username: e.target.value}))} placeholder="Reddit Username" className="bg-background border border-border/30 rounded-[2rem] px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-blue-500/30" />
                <input value={redditKeys.password} onChange={e => setRedditKeys(p => ({...p, password: e.target.value}))} placeholder="Reddit Password" type="password" className="bg-background border border-border/30 rounded-[2rem] px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-blue-500/30" />
              </div>
              <div className="flex gap-2">
                <button onClick={() => saveCredential('reddit', redditKeys)} disabled={!redditKeys.clientId || saving === 'reddit'} className="flex items-center gap-1 px-3 py-1.5 rounded-[2rem] bg-blue-500/15 border border-blue-500/25 text-blue-400 text-[10px] font-bold hover:bg-blue-500/25 disabled:opacity-40">
                  {saving === 'reddit' ? <Loader2 className="w-3 h-3 animate-spin" /> : <Check className="w-3 h-3" />} Save
                </button>
                {isConfigured('reddit') && <>
                  <button onClick={() => testCredential('reddit')} disabled={testing === 'reddit'} className="flex items-center gap-1 px-3 py-1.5 rounded-[2rem] bg-background border border-border/30 text-foreground text-[10px] font-bold hover:bg-muted/50 disabled:opacity-40">
                    {testing === 'reddit' ? <Loader2 className="w-3 h-3 animate-spin" /> : <Send className="w-3 h-3" />} Test
                  </button>
                  <button onClick={() => deleteCred('reddit')} className="flex items-center gap-1 px-3 py-1.5 rounded-[2rem] text-red-400 text-[10px] font-bold hover:bg-red-500/10"><Trash2 className="w-3 h-3" /></button>
                </>}
              </div>
            </div>

            {testResult && (
              <div className={`flex items-center gap-2 px-3 py-2 rounded-[2rem] text-xs ${testResult.success ? 'bg-green-500/10 border border-green-500/20 text-green-400' : 'bg-red-500/10 border border-red-500/20 text-red-400'}`}>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-[2rem] border border-border/40 bg-card overflow-hidden">
      <div className="px-5 py-3 border-b border-border/30 bg-emerald-500/5 flex items-center justify-between">
        ...
        <button onClick={() => setShowCreate(!showCreate)} className="text-[10px] font-bold px-2.5 py-1 rounded-[2rem] bg-emerald-500/15 border border-emerald-500/25 text-emerald-400 hover:bg-emerald-500/25">
      ...
      <div className="p-4 space-y-3">
        <AnimatePresence>
          {showCreate && (
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-3 pb-3 border-b border-border/20">
              <input value={name} onChange={e => setName(e.target.value)} placeholder="Campaign name..." className="w-full bg-background border border-border/30 rounded-[2rem] px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:border-emerald-500/30" />
              <div className="flex gap-2">
                <div className="relative flex-1">
                  <select value={theme} onChange={e => setTheme(e.target.value)} className="w-full appearance-none bg-background border border-border/40 rounded-[2rem] px-3 py-2 text-xs text-foreground pr-8 focus:outline-none">
                    {THEMES.map(t => <option key={t} value={t}>{t}</option>)}
                  </select>
                  <ChevronDown className="absolute right-2 top-1/2 -translate-y-1/2 w-3 h-3 text-muted-foreground pointer-events-none" />
                </div>
                <div className="relative flex-1">
                  <select value={frequency} onChange={e => setFrequency(e.target.value)} className="w-full appearance-none bg-background border border-border/40 rounded-[2rem] px-3 py-2 text-xs text-foreground pr-8 focus:outline-none">
              ...
              <div className="flex flex-wrap gap-1.5">
                {PLATFORMS.map(p => (
                  <button key={p} onClick={() => togglePlatform(p)} className={`px-2.5 py-1 rounded-[2rem] text-[10px] font-bold border transition-colors ${selectedPlatforms.includes(p) ? 'bg-emerald-500/15 border-emerald-500/30 text-emerald-400' : 'bg-background border-border/30 text-muted-foreground hover:border-emerald-500/20'}`}>
                    {p}
                  </button>
                ))}
              </div>
              <input value={customNote} onChange={e => setCustomNote(e.target.value)} placeholder="Optional custom note..." className="w-full bg-background border border-border/30 rounded-[2rem] px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground/50 focus:outline-none" />
              <button onClick={createCampaign} disabled={!name || !selectedPlatforms.length || creating} className="flex items-center gap-1.5 px-4 py-2 rounded-[2rem] bg-emerald-500/15 border border-emerald-500/25 text-emerald-400 text-xs font-bold hover:bg-emerald-500/25 disabled:opacity-40">
        ...
        {loading ? <div className="flex justify-center py-4"><Loader2 className="w-5 h-5 animate-spin text-muted-foreground" /></div> : campaigns.length === 0 ? (
          ...
          <div className="space-y-2">
            {campaigns.map(c => (
              <div key={c.id} className="rounded-3xl border border-border/30 p-3">
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-[2rem] border border-border/40 bg-card overflow-hidden">
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {PLATFORMS.map(p => (
            <button key={p} onClick={() => togglePlatform(p)} className={`px-2.5 py-1 rounded-[2rem] text-[10px] font-bold border transition-colors ${selectedPlatforms.includes(p) ? 'bg-amber-500/15 border-amber-500/30 text-amber-400' : 'bg-background border-border/30 text-muted-foreground hover:border-amber-500/20'}`}>
              {p}
            </button>
          ))}
        </div>
        <input value={customNote} onChange={e => setCustomNote(e.target.value)} placeholder="Optional custom angle..." className="w-full bg-background border border-border/30 rounded-[2rem] px-3 py-2 text-xs text-foreground placeholder:text-muted-foreground/50 focus:outline-none" />
        <button onClick={postNow} disabled={!selectedPlatforms.length || posting} className="flex items-center gap-1.5 px-4 py-2 rounded-[2rem] bg-amber-500/15 border border-amber-500/25 text-amber-400 text-xs font-bold hover:bg-amber-500/25 disabled:opacity-40 w-full justify-center">
        ...
        {results.length > 0 && (
          <div className="space-y-1.5 mt-2">
            {results.map((r, i) => (
              <div key={i} className={`flex items-center gap-2 px-3 py-2 rounded-[2rem] text-xs ${r.success ? 'bg-green-500/10 border border-green-500/20 text-green-400' : 'bg-red-500/10 border border-red-500/20 text-red-400'}`}>
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-[2rem] border border-border/40 bg-card overflow-hidden">
      <div className="px-5 py-3 border-b border-border/30 bg-indigo-500/5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <input type="date" value={fromDate} onChange={e => { setFromDate(e.target.value); setLoading(true); }} className="bg-background border border-border/40 rounded-[2rem] px-2 py-1 text-[10px] text-foreground focus:outline-none" placeholder="From" />
          <input type="date" value={toDate} onChange={e => { setToDate(e.target.value); setLoading(true); }} className="bg-background border border-border/40 rounded-[2rem] px-2 py-1 text-[10px] text-foreground focus:outline-none" placeholder="To" />
          <div className="relative">
            <select value={filter} onChange={e => { setFilter(e.target.value); setLoading(true); }} className="appearance-none bg-background border border-border/40 rounded-[2rem] px-2 py-1 text-[10px] text-foreground pr-6 focus:outline-none">
      ...
      <div className="p-4">
        {loading ? <div className="flex justify-center py-4"><Loader2 className="w-5 h-5 animate-spin text-muted-foreground" /></div> : logs.length === 0 ? (
          ...
          <div className="space-y-2 max-h-[400px] overflow-y-auto">
            {logs.map(log => (
              <div key={log.id} className="rounded-3xl border border-border/30 p-3">
