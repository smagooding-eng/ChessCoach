...
function GameRatingPanel({
  ...
  const PlayerAvatar = ({ username, dark, avatar }: { username: string; dark: boolean; avatar?: string }) => {
    if (avatar) {
      return <img src={avatar} alt={username} className="w-12 h-12 rounded-3xl object-cover border-2 border-white/20" />;
    }
    return (
      <div className={`w-12 h-12 rounded-3xl border-2 border-white/20 flex items-center justify-center font-black text-sm
    ...
    <div className="glass-card rounded-[2rem] overflow-hidden border border-white/8">
      ...
      <div className="grid grid-cols-[1fr_60px_1fr] items-center py-4 bg-white/3 border-t border-white/8">
        <div className="flex justify-center">
          <div className="px-4 py-2 rounded-3xl bg-background border border-white/15 min-w-[60px] text-center">
      ...
      <div className="relative max-w-[300px] mx-auto">
        ...
        {result !== 'playing' && (
          <div className="absolute inset-0 rounded-[10px] bg-black/60 flex flex-col items-center justify-center gap-2 z-10">
            ...
            <button
              onClick={resetGame}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-[2rem] bg-primary/20 hover:bg-primary/30 text-primary text-xs font-bold transition-colors"
            >
...
export function GameReplay() {
  ...
  return (
    <div className="space-y-2 md:space-y-4 pb-20 px-3 pt-3 md:px-0 md:pt-0">
      ...
      {game.opening && (
        <div className="flex glass-card rounded-[2rem] px-4 py-2.5 md:px-5 md:py-3 items-center gap-2 md:gap-3 border border-primary/20 bg-primary/5">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_340px] gap-6">
        ...
        <div className="space-y-2 md:space-y-4">
          ...
          <div className="glass-card rounded-[2rem] overflow-hidden">
            <div className="flex items-stretch">
              <div className="flex-1 flex items-center gap-2 px-3 py-2 md:py-3">
                {whitePlayer?.avatar
                  ? <img src={whitePlayer.avatar} alt={game.whiteUsername} className="w-7 h-7 md:w-9 md:h-9 rounded-[2rem] md:rounded-3xl object-cover border border-white/20 shrink-0" />
                  : <div className="w-7 h-7 md:w-9 md:h-9 rounded-[2rem] md:rounded-3xl bg-[#eeeed2] flex items-center justify-center shrink-0"><span className="text-[#2d2d2d] font-black text-xs md:text-sm">{game.whiteUsername[0]?.toUpperCase()}</span></div>
                }
                <div className="min-w-0">
              ...
              <div className="flex flex-col items-center justify-center px-2 md:px-3 border-x border-white/5 shrink-0">
                <span className={`px-2 py-0.5 md:px-2.5 md:py-1 rounded-[2rem] text-[10px] md:text-[11px] font-black uppercase tracking-wider border
              ...
              <div className="flex-1 flex items-center gap-2 px-3 py-2 md:py-3 justify-end">
                ...
                {blackPlayer?.avatar
                  ? <img src={blackPlayer.avatar} alt={game.blackUsername} className="w-7 h-7 md:w-9 md:h-9 rounded-[2rem] md:rounded-3xl object-cover border border-white/20 shrink-0" />
                  : <div className="w-7 h-7 md:w-9 md:h-9 rounded-[2rem] md:rounded-3xl bg-[#2d2d2d] border border-white/20 flex items-center justify-center shrink-0"><span className="text-[#eeeed2] font-black text-xs md:text-sm">{game.blackUsername[0]?.toUpperCase()}</span></div>
                }
              </div>
            </div>
          </div>
          ...
          {/* Playback controls */}
          <div className="glass-card rounded-[2rem] px-1.5 py-1.5 md:p-3 flex items-center justify-between">
            <div className="flex items-center">
              <button onClick={() => { setCurrentMove(0); setIsPlaying(false); }} disabled={currentMove === 0}
                className="p-2.5 md:p-2.5 rounded-3xl bg-secondary hover:bg-primary/20 hover:text-primary transition-colors disabled:opacity-40 active:scale-90">
              ...
              <button onClick={() => setIsPlaying(p => !p)}
                className="px-3.5 py-2.5 md:px-4 md:py-2.5 rounded-3xl bg-primary/10 text-primary hover:bg-primary hover:text-primary-foreground transition-colors active:scale-90">
            ...
            <div className="flex items-center gap-0.5 md:gap-2">
              ...
              <button onClick={() => setFlipped(f => !f)} title="Flip board"
                className="p-2.5 md:p-2.5 rounded-3xl bg-secondary hover:bg-primary/20 hover:text-primary transition-colors active:scale-90">
                <FlipVertical2 className="w-5 h-5 md:w-4 md:h-4" />
              </button>

              <button
                onClick={() => { setPracticeMode(p => !p); setIsPlaying(false); }}
                className={`p-2.5 md:px-3 md:py-2 rounded-3xl text-xs font-bold transition-colors border active:scale-90
                  ${practiceMode
              ...
              <button
                ...
                className={`p-2.5 md:px-3 md:py-2 rounded-3xl text-xs font-bold transition-colors border flex items-center gap-1 md:gap-1.5 active:scale-90
              ...
              {reviewMoves.length > 0 && !reviewing && (
                <button
                  ...
                  className="p-2.5 rounded-3xl text-xs font-bold transition-colors border bg-secondary border-border hover:border-primary/40 hover:text-primary active:scale-90"
                  ...
                  {currentReview ? (
                    <>
                      ...
                      {(currentReview.pros?.length > 0 || currentReview.cons?.length > 0) && (
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
                          {currentReview.pros?.length > 0 && (
                            <div className="rounded-3xl bg-emerald-500/8 border border-emerald-500/20 px-3 py-2.5">
                          ...
                          {currentReview.cons?.length > 0 && (
                            <div className="rounded-3xl bg-rose-500/8 border border-rose-500/20 px-3 py-2.5">
                      ...
                      {isBad && currentReview.betterMove && (
                        <div className="flex items-start gap-2.5 rounded-3xl bg-amber-500/8 border border-amber-500/20 px-3 py-2.5">
          ...
          {reviewing && (() => {
            ...
            return (
              <div className="space-y-3">
                <div className="glass-card rounded-3xl px-4 py-4 border border-primary/30 bg-primary/5 flex flex-col gap-3">
                ...
                {(() => {
                  ...
                  return (
                    <div className="glass-card rounded-3xl px-4 py-3 border border-white/10">
                ...
                {/* AI Mini-Game */}
                <div className="glass-card rounded-3xl p-3 border border-white/10">
          ...
          {/* Review error */}
          {reviewError && (
            <div className="glass-card rounded-3xl px-4 py-3 border border-rose-500/30 bg-rose-500/5 text-sm text-rose-400 flex items-center justify-between gap-3">
          ...
          {/* Practice mode hint */}
          {practiceMode && (
            <div className="glass-card rounded-3xl px-4 py-3 border border-emerald-500/30 bg-emerald-500/5 text-sm text-emerald-300 flex items-center gap-2">
              {fetchingBest
          ...
          {currentMove === 0 && reviewMoves.length === 0 && !reviewing && (
            <div className="glass-card rounded-3xl px-4 py-4 border border-primary/20 bg-primary/5 flex flex-col sm:flex-row items-start sm:items-center gap-3 justify-between">
              ...
              <button
                onClick={() => handleReview(false)}
                className="shrink-0 flex items-center gap-2 px-4 py-2 rounded-3xl bg-primary text-primary-foreground text-sm font-bold hover:bg-primary/90 transition-colors shadow-lg shadow-primary/20">
        ...
        {/* ── Right col: move list ── */}
        <div className="glass-card rounded-[2rem] flex flex-col xl:max-h-[700px]">
          ...
          <div ref={moveListRef} className="flex-1 overflow-y-auto p-2 hide-scrollbar">
            ...
            <div
              ...
              className={`px-3 py-1.5 rounded-[2rem] text-xs cursor-pointer transition-colors mb-1
            ...
            {Array.from({ length: Math.ceil(maxMoves / 2) }).map((_, i) => {
              ...
              const MoveBtn = ({
                ...
                return (
                  <button
                    ...
                    className={`flex-1 flex items-center gap-1 py-1.5 px-2 rounded-[2rem] font-mono text-xs text-left transition-colors
              ...
              return (
                <div key={i} className={`flex items-center gap-0.5 text-sm rounded-[2rem] ${i % 2 === 0 ? '' : 'bg-white/[0.02]'}`}>
