...
function renderStep(text: string): React.ReactNode {
  ...
  if (isMistake) {
    ...
    return (
      <div className="space-y-3">
        <div className="rounded-3xl p-4 space-y-2" style={{ background: 'rgba(220,67,67,0.1)', border: '1px solid rgba(220,67,67,0.25)' }}>
  ...
  if (isFix) {
    ...
    return (
      <div className="space-y-3">
        <div className="rounded-3xl p-4 space-y-2" style={{ background: 'rgba(129,182,76,0.1)', border: `1px solid rgba(129,182,76,0.25)` }}>
          <div className="flex items-center gap-2 mb-1">
    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        <button
          ...
          className={cn(
            'flex items-center gap-1.5 px-3 py-1.5 rounded-[2rem] text-xs font-bold transition-all',
            (speaking || loading)
              ? 'text-white'
      ...
      {steps.length > 1 && (
        <div className="flex items-center justify-between gap-3 pt-2">
          <button
            onClick={() => goTo(step - 1)}
            disabled={isFirst}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-[2rem] hover:bg-white/10 transition-all disabled:opacity-20 text-white/50 hover:text-white"
          >
  ...
  if (!course) return (
    <div className="space-y-8 pb-16 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <Link href="/courses" className="px-5 py-2 rounded-3xl bg-primary/10 hover:bg-primary/20 text-primary text-sm font-semibold transition-colors border border-primary/20">
          View all courses
        </Link>
      </div>
    </div>
  ...
  return (
    <div className="pb-20 max-w-4xl mx-auto space-y-2 md:space-y-4 px-3 md:px-0">
      ...
      <div className="flex items-center gap-2 md:gap-3">
        <Link href="/courses" className="p-2 rounded-[2rem] hover:bg-white/10 transition-colors text-white/50 hover:text-white">
      ...
      {sortedLessons.length === 0 ? (
        <div className="rounded-3xl p-12 text-center text-white/50" style={{ backgroundColor: BG_DARK }}>No lessons available.</div>
      ) : (
        <div className="flex gap-4 items-start">
          {/* Sidebar — lesson list */}
          <div className="hidden lg:flex flex-col w-56 shrink-0 rounded-3xl overflow-hidden" style={{ backgroundColor: BG_DARK }}>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {lesson && lesson.content && (
                  <div className="rounded-3xl p-3 md:p-4 mt-2 md:mt-3" style={{ backgroundColor: BG_DARK }}>
                ...
                <div className="flex flex-col sm:flex-row justify-between items-center gap-3 mt-4">
                  <button
                    ...
                    className="flex items-center gap-2 px-5 py-2 rounded-[2rem] text-sm font-bold text-white/50 hover:text-white hover:bg-white/10 transition-all disabled:opacity-20"
                  >
                  ...
                  <button
                    ...
                    disabled={isUpdating}
                    className={cn(
                      'flex items-center gap-2 px-6 py-2.5 rounded-[2rem] text-sm font-bold transition-all',
                      lesson?.completed
