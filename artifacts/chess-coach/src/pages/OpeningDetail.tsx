...
export function OpeningDetail() {
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      {/* Header */}
      <div className="glass-card rounded-[2rem] p-5 flex flex-wrap items-start justify-between gap-4">
        <div>
          {data.eco && (
      ...
      {/* Win bar */}
      <div className="glass-card rounded-3xl px-5 py-3 space-y-1.5">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-6">
        ...
        <div className="space-y-5">
          ...
          <div className="flex items-center justify-between gap-2 flex-wrap">
            ...
            {mainLine.length > 0 && (
              <div className="flex gap-2">
                {practicing ? (
                  <>
                    <div className="flex items-center gap-0.5 rounded-3xl bg-secondary p-0.5">
                      <button onClick={() => { setPracticeColor('white'); resetPractice(); }}
                        className={`px-2.5 py-1 rounded-[2rem] text-xs font-bold transition-colors ${practiceColor === 'white' ? 'bg-[#f0d9b5] text-[#2d2d2d]' : 'text-muted-foreground hover:text-foreground'}`}>
                        White
                      </button>
                      <button onClick={() => { setPracticeColor('black'); resetPractice(); }}
                        className={`px-2.5 py-1 rounded-[2rem] text-xs font-bold transition-colors ${practiceColor === 'black' ? 'bg-[#2d2d2d] text-[#f0d9b5] border border-white/20' : 'text-muted-foreground hover:text-foreground'}`}>
                        Black
                      </button>
                    </div>
                    <button onClick={resetPractice}
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-3xl bg-secondary text-muted-foreground hover:text-foreground text-sm font-medium transition-colors">
                  ...
                  <button onClick={startPractice}
                    className="flex items-center gap-1.5 px-4 py-1.5 rounded-3xl bg-primary text-primary-foreground text-sm font-bold shadow shadow-primary/30 hover:bg-primary/90 transition-colors">
          ...
          {mainLine.length === 0 ? (
            <div className="glass-card rounded-[2rem] py-16 text-center text-muted-foreground">
            ...
            <>
              {practiceComplete ? (
                <div className="glass-card rounded-[2rem] py-16 text-center space-y-4">
                  ...
                  <button onClick={resetPractice}
                    className="inline-flex items-center gap-2 px-6 py-3 rounded-3xl bg-primary text-primary-foreground font-bold hover:bg-primary/90 transition-colors">
                ...
                <>
                  ...
                  <AnimatePresence>
                    {feedback && (
                      <motion.div
                        ...
                        exit={{ opacity: 0 }}
                        className={`flex items-center gap-3 px-4 py-3 rounded-3xl font-semibold text-sm
                  ...
                  {/* Practice info */}
                  <div className="glass-card rounded-3xl px-4 py-3 flex items-center justify-between gap-4">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {mainLine.map((m, i) => (
                      <span key={i}
                        className={`px-2 py-0.5 rounded-[2rem] font-mono text-xs
            ...
            <>
              ...
              {/* Move annotation */}
              <div className="glass-card rounded-3xl px-4 py-3 min-h-[56px] flex items-center gap-3">
              ...
              <div className="glass-card rounded-3xl p-3 flex items-center justify-between gap-3">
                <div className="flex items-center gap-1 md:gap-1.5">
                  <button onClick={() => setStep(0)} disabled={step === 0}
                    className="p-3 md:p-2.5 rounded-3xl bg-secondary hover:bg-primary/20 hover:text-primary disabled:opacity-40 transition-colors active:scale-90">
                ...
                <div className="flex flex-wrap gap-1 flex-1 justify-end">
                  {mainLine.map((m, i) => (
                    <button
                      key={i}
                      onClick={() => setStep(i + 1)}
                      className={`px-2 py-0.5 rounded-[2rem] font-mono text-xs transition-colors
          ...
          {data.sampleGames.length > 0 && (
            <div className="space-y-3">
              ...
              <div className="glass-card rounded-[2rem] divide-y divide-border/50 overflow-hidden">
                {data.sampleGames.map((game, i) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`shrink-0 w-14 text-center py-1 rounded-[2rem] text-xs font-bold uppercase tracking-wide
                      ...
                      <Link href={`/games/${game.id}`}
                        className="shrink-0 flex items-center justify-center w-9 h-9 rounded-3xl bg-secondary text-primary hover:bg-primary hover:text-primary-foreground transition-all">
        ...
        <div className="space-y-4">
          ...
          {/* As white / as black */}
          {stat && (
            <div className="glass-card rounded-[2rem] p-4 space-y-3">
              ...
              <div className="space-y-2">
                {best && (
                  <Link href={`/openings/${encodeURIComponent(best.eco ?? best.opening)}`}
                    className="glass-card rounded-3xl p-3 flex items-center gap-3 hover:border-emerald-500/30 transition-colors group">
                ...
                {worst && worst.opening !== best?.opening && (
                  <Link href={`/openings/${encodeURIComponent(worst.eco ?? worst.opening)}`}
                    className="glass-card rounded-3xl p-3 flex items-center gap-3 hover:border-red-500/30 transition-colors group">
