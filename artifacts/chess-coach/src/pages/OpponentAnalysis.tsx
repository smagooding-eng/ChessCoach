...
function ScoutingWaitContent({ opponentRating }: { opponentRating?: number | null }) {
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-[2rem] bg-secondary/40 p-5 border border-white/5">
    ...
    <div className="space-y-8 pb-10 px-4 pt-4 md:px-0 md:pt-0">
      ...
      <form onSubmit={handleAnalyze} className="flex gap-3 max-w-lg">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-secondary/70 border border-border rounded-3xl text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-primary/60 transition-all"
            disabled={loading}
          />
        </div>
        <button
          ...
          className="px-6 py-3 bg-primary text-primary-foreground font-bold rounded-3xl hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors flex items-center gap-2"
        >
          ...
          <div className="flex flex-wrap gap-2">
            {scoutHistory.map(s => (
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 rounded-3xl bg-secondary/70 border border-border hover:border-primary/40 hover:text-primary transition-colors text-sm disabled:opacity-50"
              >
      ...
      {loading && statusMsg && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="p-4 rounded-3xl bg-primary/8 border border-primary/20 text-primary flex items-center gap-3"
        >
      ...
      {error && (
        <motion.div
          initial={{ opacity: 0, y: -8 }}
          animate={{ opacity: 1, y: 0 }}
          className="p-4 rounded-3xl bg-destructive/15 border border-destructive/30 text-red-400 flex items-center gap-2"
        >
        ...
        {result && !loading && (
          ...
          >
            ...
            <div className="glass-card rounded-[2rem] p-6">
                ...
                <div className="sm:ml-auto flex flex-col items-start sm:items-end justify-center gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      ...
                      disabled={loading}
                      className="flex items-center gap-1 px-2.5 py-1.5 rounded-[2rem] bg-secondary/70 border border-border hover:border-primary/40 hover:text-primary transition-colors text-xs disabled:opacity-50"
                    >
              ...
              <div className="mt-5 pt-5 border-t border-border/50">
                <a
                  ...
                  className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#81b64c] hover:bg-[#6fa03e] text-white font-bold rounded-3xl transition-colors text-sm shadow-lg"
                >
                  ...
                  <div className="space-y-3">
                    {result.topOpenings.map((o, i) => (
                      <motion.div
                        ...
                        className="glass-card rounded-3xl p-4 cursor-pointer hover:bg-white/[0.04] hover:border-primary/30 transition-all group"
                      >
            ...
            <motion.div
              ...
              className="glass-card rounded-[2rem] overflow-hidden border border-primary/20"
            >
              <div className="p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
                ...
                {courseGenState === 'idle' && (
                  <button
                    ...
                    className="shrink-0 px-5 py-2.5 rounded-3xl bg-primary text-primary-foreground font-black text-sm hover:bg-primary/90 transition-colors flex items-center gap-2"
                  >
                ...
                {courseGenState === 'done' && (
                  <Link href="/courses">
                    <div className="shrink-0 flex items-center gap-2 px-5 py-2.5 rounded-3xl bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 font-black text-sm hover:bg-emerald-500/25 transition-colors cursor-pointer">
                ...
                {courseGenState === 'error' && (
                  <div className="shrink-0 flex flex-col items-end gap-2">
                    ...
                    <button
                      onClick={handleGenerateCourses}
                      className="px-4 py-2 rounded-3xl bg-red-500/15 text-red-400 border border-red-500/30 font-bold text-xs hover:bg-red-500/25 transition-colors"
                    >
                      Retry
                    </button>
                  </div>
                ...
                <div className="px-5 pb-4 pt-0">
                  <div className="flex flex-wrap gap-2">
                    {result.weaknesses.slice(0, 3).map((w, i) => (
                      <span key={i} className="px-2.5 py-1 rounded-[2rem] bg-primary/10 text-primary text-xs font-semibold">
